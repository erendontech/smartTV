...
CREATE PROCEDURE `getMoviesAllData`()
BEGIN
	...
	JOIN m_quality q on m.quality_id = q.id
	JOIN m_director d on m.director_id = d.id
	JOIN (SELECT mg.id_movie as mgid ,group_concat(g.id,'-',g.name) as 'genres' from movie_genre mg
	JOIN m_genre g on mg.id_genre = g.id group by mg.id_movie) g ON m.id = g.mgid
	JOIN (SELECT mc.id_movie as mcid, group_concat(c.id,'-',c.name) as 'cast' from movie_cast mc
	JOIN m_cast c on mc.id_cast = c.id group by mc.id_movie) c ON m.id = c.mcid;
